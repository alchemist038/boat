...
def test_load_runtime_profiles_includes_disabled_shared_l1_234_candidate() -> None:
    profiles = runtime.load_runtime_profiles(include_disabled=True)
    profile_map = {profile.profile_id: profile for profile in profiles}

    assert "l1_weak_234_box_v1" in profile_map
    profile = profile_map["l1_weak_234_box_v1"]
    assert profile.source_kind == "shared"
    assert profile.enabled is False
...
def test_build_runtime_watchlist_row_supports_shared_l1_234_candidate() -> None:
    profile = next(
        profile
        for profile in runtime.load_runtime_profiles(include_disabled=True)
    ...
    assert row is not None
    assert row["profile_id"] == "l1_weak_234_box_v1"
    assert row["strategy_id"] == "l1_234"
    assert row["status"] == "waiting_beforeinfo"
