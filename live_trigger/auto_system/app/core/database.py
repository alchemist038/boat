...
def _next_legacy_name(connection: sqlite3.Connection, base_name: str) -> str:
    existing = {
        str(row[0])
...
def _rename_table_if_needed(connection: sqlite3.Connection, table_name: str, *, required_columns: set[str]) -> None:
    existing_tables = {
        str(row[0])
        for row in connection.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()
    }
    ...
    legacy_name = _next_legacy_name(connection, f"legacy_{table_name}")
...
def prepare_database() -> None:
    ...
    try:
        _rename_table_if_needed(connection, "target_races", required_columns={"target_key", "deadline_at", "payload_json"})
        _rename_table_if_needed(connection, "bet_history", required_columns={"deprecated"})
        connection.commit()
    finally:
        connection.close()
