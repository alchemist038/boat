...
- from: me
- status: active
- updated_at: 2026-04-20 07:00 JST
- priority: high
...
Current active set on `live_trigger_cli` is now the six profiles listed below.
Point-in-time forward tracking lives at:
- `reports/live_trade/live_trigger_cli_forward_logic_performance_latest/README.md`
...
昨日の shared DB repair 内容です。

### A. Shared DB Repair

- shared canonical DB は現在こうです。
  - `races / entries / beforeinfo_entries / race_meta`: `2026-04-14` まで
  - `results / odds_2t / odds_3t`: `2026-04-13` まで
- `2026-04-12` は targeted repair で復旧済みです。
  - `race_count=132`
  - `beforeinfo_entry_count=792`
  - `odds_2t_count=5940`
  - `odds_3t_count=15840`
  - `result_count=132`
- `2026-04-13` も同件数で揃っています。

### B. Root Cause And Fix

- 問題は share 切れそのものより、cached raw の再利用でした。
- `results` の cached raw が空ページのまま残るケース
- `odds_3t` の cached raw が zero-row のまま残るケース
がありました。
- `src/boat_race_data/cli.py` に次を追加済みです。
  - cached `result` が空なら 1 回だけ force refetch
  - cached `odds_3t` が zero-row なら 1 回だけ force refetch

### C. Remaining Known Odds Issue

- その後 `2026-03-27` の `odds_3t=0` も targeted `collect-day` で復旧済みです。
- 現在の shared canonical DB は:
  - `races / entries / beforeinfo_entries / race_meta`: `2026-04-15` まで
  - `results / odds_2t / odds_3t`: `2026-04-14` まで
- いまの残課題は「大きな直近欠損」ではなく、今後の partial mismatch forensic cleanup 側です。
