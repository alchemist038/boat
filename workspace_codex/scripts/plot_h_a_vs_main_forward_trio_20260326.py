...
OUTPUT_DIR = (
    ...
    / "strategies"
    / "combined"
    / "h_a_vs_main_forward_trio_2025-04-01_to_2026-03-09_20260326"
)
...
def write_note(path: Path, summary_df: pd.DataFrame) -> None:
    lines = [
        "# H-A vs Main Forward Trio Equity Overlay",
        "",
        "## Scope",
        "",
        f"- aligned period: `{START_DATE}` .. `{END_DATE}`",
        "- current trio used in this note:",
        ...
        "## Plot Files",
        "",
        "- `h_a_vs_main_forward_trio_equity.png`",
        "- `h_a_vs_main_forward_trio_equity_summary.csv`",
        "- `h_a_vs_main_forward_trio_equity_race_results.csv`",
        "",
        "## Assumptions",
        "",
...
def plot_series(series_list: list[StrategySeries], out_path: Path) -> None:
    ...
    fig.suptitle("H-A vs Main Forward Trio | 2025-04-01 .. 2026-03-09", fontsize=15)
    fig.autofmt_xdate()
    fig.tight_layout()
...
def main() -> None:
    ...
    combined.to_csv(
        OUTPUT_DIR / "h_a_vs_main_forward_trio_equity_race_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    summary_df.to_csv(
        OUTPUT_DIR / "h_a_vs_main_forward_trio_equity_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    write_note(OUTPUT_DIR / "README.md", summary_df)
    plot_series(series_list, OUTPUT_DIR / "h_a_vs_main_forward_trio_equity.png")
